...
COLOR_DISCONNECTED=$inactive
#COLOR_NEWDEVICE='#ff0'          # New Device
COLOR_NEWDEVICE=$alert
...
COLOR_BATTERY_90=$fg_alt
COLOR_BATTERY_80=$fg_alt
COLOR_BATTERY_70=$fg_alt
COLOR_BATTERY_60=$fg_alt
COLOR_BATTERY_50=$fg_alt
COLOR_BATTERY_LOW=$fg_alt
...
show_menu () {
    menu="$(rofi -sep "|" -dmenu -i -p "$DEV_NAME" <<< "Battery: $DEV_BATTERY%|Ping|Find Device|Send File|Browse Files|Unpair")"
    case "$menu" in
...
show_pmenu () {
    menu="$(rofi -sep "|" -dmenu -i -p "$DEV_NAME" <<<"Pair Device")"
    case "$menu" in
...
show_pmenu2 () {
    menu="$(rofi -sep "|" -dmenu -i -p "$1 has sent a pairing request" <<< "Accept|Reject")"
    case "$menu" in
